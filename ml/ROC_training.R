...
set.seed(1)
sl_lasso1 = SuperLearner(Y = y_train1, X = x_train1, family = binomial(),
                        SL.library = "SL.glmnet")
sl_lasso1

predictions <- predict.SuperLearner(sl_lasso1, newdata=x_holdout1)
...
y_holdout1=as.factor(y_holdout1)
library(e1071)
library(caret)
cm1<- confusionMatrix(conv.preds1,y_holdout1)
cm1

library(pROC)
modelroc1 <- roc(conv.preds1,as.numeric(y_holdout1))
plot(modelroc1, print.auc=TRUE, auc.polygon=TRUE,legacy.axes=TRUE, grid=c(0.1, 0.2),lty=2,
     grid.col=c("green", "red"), max.auc.polygon=TRUE,print.thres.adj=c(-.05,1.25),
...
set.seed(1)
sl_lasso2 = SuperLearner(Y = y_train2, X = x_train2, family = binomial(),
                         SL.library = "SL.glmnet")
sl_lasso2

predictions <- predict.SuperLearner(sl_lasso2, newdata=x_holdout2)
head(predictions$pred)
head(predictions$library.predict)
library(dplyr)
conv.preds2 <- ifelse(predictions$pred>=0.5, 1, 0)
conv.preds2=as.factor(conv.preds2)
y_holdout2=as.factor(y_holdout2)
library(e1071)
library(caret)
cm2<- confusionMatrix(conv.preds2,y_holdout2)
cm2

library(pROC)
modelroc2 <- roc(conv.preds2,as.numeric(y_holdout2))
...
set.seed(1)
sl_lasso3 = SuperLearner(Y = y_train3, X = x_train3, family = binomial(),
                         SL.library = "SL.glmnet")
sl_lasso3

predictions <- predict.SuperLearner(sl_lasso3, newdata=x_holdout3)
head(predictions$pred)
head(predictions$library.predict)
library(dplyr)
conv.preds3 <- ifelse(predictions$pred>=0.5, 1, 0)
conv.preds3=as.factor(conv.preds3)
y_holdout3=as.factor(y_holdout3)
library(e1071)
library(caret)
cm3<- confusionMatrix(conv.preds3,y_holdout3)
cm3

library(pROC)
modelroc3 <- roc(conv.preds3,as.numeric(y_holdout3))
...
set.seed(1)
sl_lasso4 = SuperLearner(Y = y_train4, X = x_train4, family = binomial(),
                         SL.library = "SL.glmnet")
sl_lasso4

predictions <- predict.SuperLearner(sl_lasso4, newdata=x_holdout4)
head(predictions$pred)
head(predictions$library.predict)
library(dplyr)
conv.preds4 <- ifelse(predictions$pred>=0.5, 1, 0)
conv.preds4=as.factor(conv.preds4)
y_holdout4=as.factor(y_holdout4)
library(e1071)
library(caret)
cm4<- confusionMatrix(conv.preds4,y_holdout4)
cm4

library(pROC)
modelroc4 <- roc(conv.preds4,as.numeric(y_holdout4))
...
(AUC = 0.875)", "Gaussian 
(AUC = 0.871)","LOG 
(AUC = 0.876)","Neighbouring
(AUC = 0.844)"), lty = 4, col = c("black", "green", "red","blue"),bty = "n")
